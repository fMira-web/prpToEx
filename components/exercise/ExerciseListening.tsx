...
export default function ExerciseListening({
  ...
  function handleSubmit() {
    const result = gradeQuestions(exercise.questions, given);
    setGraded(result);
    recordExerciseAttempt(dayNum, 'listening', {
      scorePercent: result.total ? Math.round((result.correct / result.total) * 1000) / 10 : 0,
      answers: given,
    });
  }
