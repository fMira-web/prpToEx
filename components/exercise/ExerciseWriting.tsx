...
export default function ExerciseWriting({
  ...
  const { recordExerciseAttempt } = useRoadmap();
  ...
  // Writing has no auto-grader — record whatever was actually written plus
  // the self-assessment checklist, so the history table has something to
  // show even without a numeric score (bandEstimate stays unset here; a
  // future AI-writing-grader could fill it in the same way Speaking does).
  function handleFinish() {
    recordExerciseAttempt(dayNum, 'writing', {
      answers: { mainText, extraAnswers, checklistChecked: checked },
    });
    onFinish();
  }
  ...
  return (
    <>
      ...
      <CompletionFooter dayNum={dayNum} onFinish={handleFinish} />
    </>
  );
}
